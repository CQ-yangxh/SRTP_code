...
for line in lines:
    ...
    if(line_temp[10]!='0.0000000'):#清除没有定位的数据
        data.append(line_temp)

f.close()

txt=''
with open('2021-02-07-bd.txt','w') as f:
    for line in data:
        bd_point=wgs2bd(float(line[9]),float(line[10]))
        line[9]=str(bd_point[0])
        line[10]=str(bd_point[1])
